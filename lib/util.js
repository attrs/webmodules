    ...
    return function(basedir, excludes) {
      ...
      function visit(dir, nomore) {
        ...
        list.forEach(function(filename) {
          ...
          if( filename.startsWith('._') || filename.endsWith('.swp') || filename.endsWith('.log') || filename.startsWith('.wafpickle') ) return;
          
          var file = path.join(dir, filename);
          var stat = fs.statSync(file);
          if( stat.isDirectory() ) dirs.push(file.substring(basedir.length)), !nomore && visit(file, ~['node_modules', 'web_modules'].indexOf(filename));

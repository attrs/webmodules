...
module.exports = {
  install: function(pkg, options, done) {
    ...
    bower.commands
    .install([expression], { save:false, force: true, allow_root: true }, { interactive: true, directory: tmp })
    .on('log', function(log) {
      console.log('\t%s %s', chalk.green(log.id), log.message);
    })
    ...
    .on('end', function (installed) {
      ...
      manifest.bowerDependencies = bower_manifest.dependencies;

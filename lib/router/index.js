      ...
      fs.readdirSync(map).forEach(function(filename) {
        ...
        if( pkgname[0] === '@' ) {  // private npm
          fs.readdirSync(pkgdir).forEach(function(_filename) {
            var _pkgname = pkgname + '/' + _filename;
            var _pkgdir = path.join(pkgdir, _filename);
            if( _pkgname[0] === '.' || !fs.statSync(_pkgdir).isDirectory() ) return;
            pkg_dir_map[_pkgname] = _pkgdir;
            pkg_router_map[_pkgname] = serveStatic(_pkgdir);
          });
        } else {

...
var serveModules = function(map) {
  ...
  return function(req, res, next) {
    ...
    var m = modules[pkgname] || (function() {
      var dir;
      if( typeof map === 'string' && pkgname ) dir = path.join(map, pkgname);
...
var emptyrouter = function(req, res, next) {
  next();
};
...
module.exports = function(options) {
  ...
  var rcfile = options.rcfile || path.join(process.cwd(), '.webmodulesrc');
  var rc = options.rc || fs.existsSync(rcfile) ? JSON.parse(fs.readFileSync(rcfile)) : {};
  ...
  var node_modules_path = options.node_modules === false ? false : (options.node_modules || path.join(cwd, 'node_modules'));
  var web_modules_path = options.web_modules === false ? false : (options.web_modules || rc.directory || path.join(cwd, 'web_modules'));
  ...
  var serve_node_modules = node_modules_path ? serveModules(node_modules_path) : emptyrouter;
  var serve_web_modules = web_modules_path ? serveModules(web_modules_path) : emptyrouter;

...
module.exports = function(options) {
  ...
  var serve_node_modules = serveStatic(node_modules_path);
  var serve_web_modules = serveStatic(web_modules_path);
  ...
  var cwd_manifestmap, runtime_manifestmap;
  
  return function webmodules(req, res, next) {
    if( arguments.length <= 2 ) next = finalhandler(req, res);
    
    if( !cwd_manifestmap ) cwd_manifestmap = extract(cwd);
    if( !runtime_manifestmap ) runtime_manifestmap = extract(runtime_path);
    ...
    if( serve_self && req.url.startsWith('/' + pkg_name + '/') ) {
      ...
      req.url = req.url.substring('/webmodules/'.length - 1);

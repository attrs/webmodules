...
module.exports = function uninstall(pkgs, options, done) {
  ...
  async.eachSeries(pkgs, function(name, done) {
    ...
    if( options.save && fs.existsSync(pkgfile) ) {
      ...
      var wd = manifest.browserDependencies = manifest.browserDependencies || manifest.webDependencies;
      if( wd ) {
      ...
      if( manifest.browserDependencies && !Object.keys(manifest.browserDependencies).length ) delete manifest.browserDependencies;
      if( manifest.webDependencies && !Object.keys(manifest.webDependencies).length ) delete manifest.webDependencies;

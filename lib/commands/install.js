...
module.exports = function install(pkgs, options, done) {
  ...
  if( !pkgs || !pkgs.length ) {
    ...
    pkgs = extractDependencies(require(pkgfile).webDependencies);
    options.save = false;
  }
  ...
  async.eachSeries(pkgs, function(pkg, done) {
    ...
    resolver.install(pkg, {
      ...
      function next(err) {
        ...
        var pkgs = extractDependencies(pkg.manifest && pkg.manifest.webPeerDependencies);
      ...
      var pkgs = extractDependencies(pkg.manifest && pkg.manifest.webDependencies);
